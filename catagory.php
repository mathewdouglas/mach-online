...
// Search database for catogory with given id
$sql = "SELECT * FROM odfu_catagories WHERE id = $search_id";
$res = mysqli_query($connection, $sql);

if (mysqli_num_rows($res) > 0) {
    $sql_result = mysqli_fetch_assoc($res);
    $catagory = $sql_result['catagory_name'];
    $catagory_id = $sql_result['id'];
    $img_small = $sql_result['img_small'];
    $img_medium = $sql_result['img_medium'];
}

// Search database for services with given catagory id
$sql2 = "SELECT * FROM odfu_services WHERE catagory_id = $search_id";
...
<body ondragstart="return false">
    ...
    <div class="top-section">
        <p class="catagory-title"><?php echo $catagory?></p>
    </div>
    <div class="card-grid">
        <?php
            if (mysqli_num_rows($res2) > 0) {
                while ($sql_result2 = mysqli_fetch_assoc($res2)) {
                    $sql_result2['id'];
                    echo '<div class="item"><a class="item-link" href="service.php?sid='.$sql_result2["id"].'">
                    <h2 class="item-title">'.$sql_result2["service_title"].'</h2>
                    <img class="item-image" src="Assets/'.$sql_result2["img_small"].'" alt="">
...
<style>
    .item {
        height: calc((100vw - 270px) / 5 * 0.624);
        width: calc((100vw  - 270px) / 5);
        background-color: #f2f2f7;
        border-radius: calc((100vw - 270px) / 80);
    ...
    .overlay {
        position: relative;
        top: calc((100vw - 270px) / 5 * -0.624 + 5px *-1);
        height: calc((100vw  - 270px) / 5 * 0.624);
        width: calc((100vw  - 270px) / 5);
        background: linear-gradient(rgba(0,0,0,0), rgba(0,0,0,0), rgba(0,0,0,0), rgba(0,0,0,60%));
        border-radius: calc((100vw - 270px) / 80);
    }
    ...
    .item-title {
        ...
        height: 0;
        margin: 0;
        top: calc((100vw  - 270px) / 5 * 0.624 - 35px);
        left: 20px;
        text-shadow: 0px 5px 6px rgba(0, 0, 0, 20%);
    }
